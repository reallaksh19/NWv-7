# Insight Prefetch Quality Report

- Status: **WARN**
- Schema: `3`
- Collector: `insight-collector-json-v4`
- Content hash: `091b439c72d1a389`
- Stories: `560`
- Usable 36h stories: `522`
- Source groups: `10`
- Angle hint coverage: `100%`
- Non-base angle stories: `376`
- Event sketches: `5`
- Multi-source sketches: `1`

## Slot health

| Slot | Story IDs | Linked | Sources | Thin |
|---|---:|---:|---:|---|
| now | 60 | 60 | 3 | False |
| minus4h | 40 | 40 | 2 | True |
| minus12h | 80 | 80 | 5 | False |
| minus24h | 55 | 55 | 3 | False |

## Warnings

- Weak multi-source event sketch coverage: 1 sketches < recommended 2
- feed 'ndtv' has returned zero items for 37 consecutive runs
- feed 'financial_express' has returned zero items for 179 consecutive runs
- feed 'muscat_daily' has returned zero items for 163 consecutive runs (last zero at 1782518612730)

## Top angles

- base_report: 184
- official_response: 147
- fact_update: 77
- market_reaction: 62
- regional_followup: 41
- investigative_detail: 25
- reaction_public: 10
- expert_analysis: 10
- correction: 2
- background_context: 2
