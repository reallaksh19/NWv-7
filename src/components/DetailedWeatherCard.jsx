...
export default function DetailedWeatherCard({ weatherData, activeCity, setActiveCity }) {
    ...
    const locationOptions = getWeatherLocationOptions();
    const locationByKey = Object.fromEntries(locationOptions.map(opt => [opt.key, opt]));

    const cityLabels = Object.fromEntries(
        cities.map(city => [city, getWeatherLocationLabel(city)])
    );

    const cityIcons = Object.fromEntries(
        cities.map(city => [city, locationByKey[city]?.icon || '📍'])
    );
