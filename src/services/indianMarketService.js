        ...
        try {
            const withTimeout = (promise, ms) => {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    timeoutId = setTimeout(() => reject(new Error('Orchestrator timeout')), ms);
                });
                return Promise.race([promise, timeoutPromise]).finally(() => clearTimeout(timeoutId));
            };
            const [indices, mutualFunds, commodities, currencies, ipo, nfo, stockCategories] = await Promise.allSettled([
                withTimeout(fetchIndices(), 15000),
                withTimeout(fetchMutualFunds(), 15000),
                withTimeout(fetchCommodities(), 15000),
                withTimeout(fetchCurrencyRates(), 15000),
                withTimeout(fetchIPOData(), 15000),
                withTimeout(fetchNFOData(), 15000),
                withTimeout(fetchStockCategories(), 15000)
            ]);
            const result = {
...
export default { fetchAllMarketData, fetchStaticSnapshot, fetchIndices, fetchMutualFunds, fetchIPOData, fetchTopMovers, fetchSectoralIndices, fetchCommodities, fetchCurrencyRates, fetchFIIDII, fetchNFOData, fetchStockCategories };
