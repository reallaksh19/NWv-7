...
const plannerStorage = {
    ...
    addItem: (date, item) => {
        try {
            ...
            // ── FIX: reject items with no resolvable date ──────────────────────────
            // eventDateKey === 'nodate' means normalizeDateKey() could not parse the
            // date argument. Writing under 'nodate' pollutes localStorage permanently
            // because 'nodate' is not a valid ISO date and evades all date-based pruning.
    ...
    savePlanToApi: async (planData) => {
        try {
            ...
            if (!response.ok) {
                // This is the silent failure mode that existed before.
                // Now it surfaces in the console so sync failures are visible.
                console.warn(
